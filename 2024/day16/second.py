        ...
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            ...
            for direction in directions:
                ...
                if straight_score < best_score:
                    ...
                    next_state[(*position, direction)] = set([(*straight_position, direction)])
                elif straight_score == best_score and best_score != float('inf'):
                    next_state[(*position, direction)].add((*straight_position, direction))
                ...
                for turn in turns_90_degrees(direction):
                    ...
                    if turn_score < best_score:
                        ...
                        next_state[(*position, direction)] = set([(*turn_position, turn)])
                    elif turn_score == best_score and best_score != float('inf'):
                        next_state[(*position, direction)].add((*turn_position, turn))
...
def paint_map(position):
    if position == 'win':
        return
    for x, y, direction in position:
        print(x, y, direction, next_state[(x, y, direction)])
        solved_maze[y] = solved_maze[y][:x] + "O" + solved_maze[y][x+1:]
        paint_map(next_state[(x, y, direction)])
...
for y, row in enumerate(maze_map):
    for x, cell in enumerate(row):
        ...
        if cell == 'S':
            ...
            paint_map(_next_state)

print("\n".join(solved_maze))
path_tiles_count = sum([row.count("O") for row in solved_maze]) + 1
print(path_tiles_count)
